...
import { api, useAutoObservable, split, constant, isEmpty, forEach, copyObject, produce, startsWith } from '@/common/utils';
...
export default (props) => {
    ...
    const getPosition = (id,tid) => {
        zip(api.tposition.getPosition(id), api.tposition.listEntrusByPosition(id,tid)).subscribe({
            next: ([data, data2]) => {
                setPermGroupOrUserId(data2);
                const position = data[0];
                const sparams = window.opener.onGetParams();
                // sparams.tenantId = tid;
    ...
    useEffect(() => {
        const [positionId, tenantId] = split(params.id, '_');
        if (!startsWith('ADD',params.id)) {
            getPosition(positionId, tenantId);
        } else {
            const sparams = window.opener.onGetParams() || {};
            sparams.tenantId = tenantId;
            setCurrent(sparams);
        }
        treeAllGroupsAndUsers(tenantId);
    }, [params.id]);

    const treeAllGroupsAndUsers = (tid) => {
    ...
    const onSaveClick = (position) => {
        ...
        api.tposition.saveOrUpdatePosition(position).subscribe({
            next: () => {
