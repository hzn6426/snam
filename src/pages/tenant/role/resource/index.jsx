...
import { api, contains, copyObject, forEach, forEachObject, groupBy, isEmpty, mapObjIndexed, produce, useAutoObservable, useAutoObservableEvent, split } from '@/common/utils';
...
import role from '@/pages/group/role';
...
export default (props) => {
    ...
    //     inputs$.pipe(
    //         map(([id]) => id),
    //         switchMap((id) => zip(api.trole.treeAllMenus(), api.trole.listPermMenus(id))),
    //         map(([menus, permMenus]) => {
    //             addIcon(menus);
    //             setTreeData(menus);
    ...
    useEffect(() => {
        ...
        const [roleId, tenantId] = split(params.id,'_')
        zip(api.trole.treeAllMenus(tenantId), api.trole.listPermMenus(roleId, tenantId)).subscribe({
            next:([menus, permMenus]) => { 
                addIcon(menus);
                setTreeData(menus);
                setUserPerms(permMenus);
                setCurrent({roleId:roleId, tenantId:tenantId})
            }
        })
    },[params.id])
    ...
    const [onClickMenu] = useAutoObservableEvent(
        [
            tap(() => setLoading(true)),
            switchMap(([menuId, roleId, tenantId]) => zip(api.trole.listAllButtonsByMenu(menuId, tenantId), api.trole.listPermButtons(roleId, menuId, tenantId))),
    ...
    const onMenuSave = (privileges) => {
        api.trole.saveMenuPerm(privileges).subscribe({
            next: () => {
    ...
    const onButtonSave = (privileges) => {
        api.trole.saveButtonPerm(privileges).subscribe({
            next: () => {
                message.success('操作成功!');
            }
        })
                            ...
                            <div style={{ float: 'right', paddingRight: '10px' }}>
                                <Space>
                                    <Button
                                        ...
                                        onClick={() => {
                                            const [roleId, tenantId] = split(params.id, '_');
                                            const privileges = { menus: userPerms, roleId: roleId, tenantId: tenantId };
                    ...
                    <Tree
                        ...
                        onSelect={(keys, { selected, node }) => {
                            if (selected && node.isLeaf) {
                                onClickMenu([keys[0], current.roleId, current.tenantId]);
