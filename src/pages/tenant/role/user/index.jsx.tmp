...
export default (props) => {
    ...
    const loadGroupUsersByRole = (roleId, tid) => {
        api.tuser.listGroupUsersByRole(roleId, tid).subscribe({
            next:(data) => {
                const ids = [];
                forEach((v) => {
                    let groups = [];
    ...
    useEffect(() => {
        ...
        loadGroupUsersByRole(roleId, tenantId);
    ...
    const [onSaveClick] = useAutoObservableEvent([
        ...
        switchMap((userRole) => api.trole.saveUserRole(userRole)),
        tap(() => {
