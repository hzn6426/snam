...
export default (props) => {
    ...
    const [current, setCurrent] = useAutoObservable((inputs$) =>
        inputs$.pipe(
            map(([id]) => id),
            switchMap((id) => zip(api.role.treeAllTenantMenus(), api.tenant.listAllMenus(id))),
            map(([menus, permMenus]) => {
                addIcon(menus);
                setTreeData(menus);
    ...
    const [onClickMenu] = useAutoObservableEvent(
        [
            tap(() => setLoading(true)),
            switchMap(([menuId, tenantId]) => zip(api.role.listAllTenantButtonsByMenu(menuId), api.tenant.listAllButtonsByMenu(tenantId, menuId))),
    ...
    const onMenuSave = (privileges) => {
        api.tenant.assignMenus(privileges).subscribe({
            next: () => {
    ...
    const onButtonSave = (privileges) => {
        api.tenant.assignButtons(privileges).subscribe({
            next: () => {
                message.success('操作成功!');
            }
        })
                            ...
                            <div style={{ float: 'right', paddingRight: '10px' }}>
                                <Space>
                                    <Button
                                        ...
                                        icon={<SaveOutlined />}
                                        onClick={() => {
                                            const privileges = { ids: userPerms, tenantId: params.id };
                    ...
                    <Tree
                        ...
                        onSelect={(keys, { selected, node }) => {
                            if (selected && node.isLeaf) {
                                onClickMenu([keys[0], current.tenantId]);
