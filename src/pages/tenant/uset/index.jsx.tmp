...
export default (props) => {
    ...
    const handleUse = () => {
        setLoading(true);
        api.tuset.useUset(selectedKeys).subscribe({
            next: () => {
    ...
    const handleStop = () => {
        setLoading(true);
        api.tuset.stopUset(selectedKeys).subscribe({
            next: () => {
                message.success('操作成功!');
                search(pageNo, pageSize);
            }
    ...
    const handleDelete = () => {
        setLoading(true);
        api.tuset.deleteUset({ids:selectedKeys,tenantId:props.tenantId}).subscribe({
            next: () => {
                message.success('操作成功!');
                search(pageNo, pageSize);
            }
    ...
    const handleAssignUsers = () => {
        ...
        INewWindow({
            url: '/new/tuset/user/' + usetId + '_' + props.tenantId,
            title: '添加用户',
            width: 700,
            height: 600,
    ...
    const handleAssignRoles = () => {
        ...
        const usetId = selectedKeys[0];
        INewWindow({
            url: '/new/tuset/role/' + usetId + '_' + props.tenantId,
            title: '分配角色',
            width: 700,
            height: 600,
            callback: () => search(pageNo, pageSize)
    ...
    const onDoubleClick = (id) => {
        INewWindow({
            url: '/new/tuset/' + id + '_' + props.tenantId,
            title: '编辑用户组',
            width: 600,
            height: 300,
            callback: () => search(pageNo, pageSize)
    ...
    const onNewClick = () => {
        INewWindow({
            url: '/new/tuset/ADD' + '_' + props.tenantId,
            title: '新增用户组',
            width: 600,
            height: 300,
            callback: () => search(pageNo, pageSize)
    ...
    const search = (pageNo, pageSize) => {
        ...
        api.tuset.searchUset(param).subscribe({
            next: (data) => {
