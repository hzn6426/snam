...
export default (props) => {
    ...
    const treeAllGroupsAndUsers = (tenantId) => {
        api.tgroup.treeAllGroupsAndUsers(tenantId).subscribe({
            next: (data) => {
                setTreeData(data);
            }
        })
    ...
    const loadGroupUsersByUset = (usetId, tenantId) => {
        api.tuser.listGroupUsersByUset(usetId, tenantId).subscribe({
            next: (data) => {
                const ids = [];
                forEach((v) => {
                    let groups = [];
    ...
    useEffect(() => {
        ...
        setCurrent({usetId:usetId, tenantId:tenantId});
        loadGroupUsersByUset(usetId, tenantId);
    ...
    const [onSaveClick] = useAutoObservableEvent([
        ...
        map(([userUset, userIds]) => {
            ...
            copyObject(userUset, { groupUsers });
            return userUset;
        }),
        switchMap((userUset) => api.tuset.saveUserUset(userUset)),
        tap(() => {
