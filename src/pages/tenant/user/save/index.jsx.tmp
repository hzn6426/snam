...
import { api, constant, data2Option, isArray, split, useAutoObservable,startsWith } from '@/common/utils';
...
export default (props) => {
    ...
    //     inputs$.pipe(
    //         map(([id]) => id),
    //         filter(id => !startsWith('ADD',id)),
    //         switchMap((id) => api.tuser.getUser(id)),
    //         map((t1) => {
    //             const user = t1[0]
    ...
    useEffect(() => { 
        if (params.id) {
            ...
            if (startsWith('ADD',id)) {
                ...
                api.tuser.getUser(uid).subscribe({
                    next: (data) => {
                        const user = data[0]
                        // if (user.beMultiLogin === true) {
                        //     user.loginMode = 'SHARED';
    ...
    const onSaveClick = (user) => {
        ...
        api.tuser.saveOrUpdateUser(user).subscribe({
            next: () => {
    ...
    const loadUserTags = () => {
        api.dict.listChildByParentCode(constant.DICT_TENANT_BUSINEESS_TAG).subscribe({
            next: (t2) => {
    ...
    const loadSystemPointTags = () => {
        api.dict.listChildByParentCode(constant.DICT_TENANT_POINT_TAG).subscribe({
            next: (t2) => {
                const dicts = data2Option('dictCode', 'dictName', t2);
