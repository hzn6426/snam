...
export default (props) => {
    ...
    const loadGroup = (tid) => {
        api.tgroup.treeAllGroupsAndUsers(tid).subscribe({
            next: (data) => {
                setTreeData(data);
    ...
    // 根据用户加载对应的权限资源
    const loadPermResourceByUser = (orgId, uid, tid) => {
        api.tresource.listBPermResourcesByUser(orgId, uid, tid).subscribe({
            next: (data) => setDataSource(data),
        });
    };

    // 根据用户组加载对应的权限资源
    const loadPermResourceByUset = (usetId, tid) => {
        api.tresource.listBPermResourcesByUset(usetId, tid).subscribe({
            next: (data) => setDataSource(data),
        });
    };

    // 根据用户及权限加载对应的业务权限
    const loadUserPermEntrusts = (orgId, uid, permId, tid) => {
        zip(
            api.tresource.listPermEntrustsByUser(orgId, uid, permId, tid),
            api.tresource.listPermExceptEntrustsByUser(orgId, uid, permId, tid),
        ).subscribe({
            next: ([data1, data2]) => {
                forEach((v) => {
                    v.disabled = false;
    ...
    // 根据用户组及权限加载对应的业务权限
    const loadUsetPermEntrusts = (usetId, permId, tid) => {
        zip(
            api.tresource.listPermEntrustsByUset(usetId, permId, tid),
            api.tresource.listPermExceptEntrustsByUset(usetId, permId, tid)
        ).subscribe({
            next: ([data1, data2]) => {
                forEach((v) => {
                    v.disabled = false;
    ...
    const resetUserPerm = () => {
        ...
        if (permType === 'user') {
            loadUserPermEntrusts(selectedUserGroupId, selectedUserId, selectedPermId, props.tenantId);
        } else if (permType === 'uset') {
            loadUsetPermEntrusts(selectedUsetId, selectedPermId, props.tenantId);
        }
    };
    ...
    // 根据用户获取对应的数据权限
    const loadUserDataPerm = (userId, orgId, permId, tid) => {
        api.tresource.loadUserDataPerm(userId, orgId, permId, tid).subscribe({
            next: (data) => {
    ...
    // 根据用户组获取对应的数据权限
    const loadUsetDataPerm = (usetId, permId, tid) => {
        api.tresource.loadUsetDataPerm(usetId, permId, tid).subscribe({
            next: (data) => {
                // if (!data || data.length === 0) {
                //     return;
                // }
    ...
    const submitUserDataPerm = () => {
        ...
        bform.validateFields().then((values) => {
            ...
            if (permType === 'user') {
                setSaveLoading(true);
                api.tresource.saveUserDataPerm(values).subscribe({
                    next: () => {
                ...
                setSaveLoading(true);
                api.tresource.saveUsetDataPerm(values).subscribe({
                    next: () => {
                        message.success("操作成功!");
                    }
                }).add(() => setSaveLoading(false));
    ...
    const loadTablesByPerm = (userId, orgId, permId, callback) => {
        api.tresource.fetchActionMapperByPerm(permId).subscribe({
            next: (data) => {
    ...
    const loadColumnPermTablesByPerm = (permId, callback) => {
        api.tresource.fetchColumnActionMapperByPerm(permId).subscribe({
            next: (data) => {
                const result = data && data[0];
                const options = [];
                const aliasMap = {};
    ...
    //加载用户列权限
    const loadUserColumnPerm = (userId, orgId, permId, tid) => {
        api.tresource.loadUserColumnPerm(userId, orgId, permId, tid).subscribe({
            next: (data) => {
                if (!data || data.length === 0) {
                    return;
                }
    ...
    //加载用户组列权限
    const loadUsetColumnPerm = (usetId, permId, tid) => {
        api.tresource.loadUsetColumnPerm(usetId, permId, tid).subscribe({
            next: (data) => {
                if (!data || data.length === 0) {
                    return;
                }
    ...
    const submitColumnPerm = () => {
        ...
        cform.validateFields().then((values) => {
            ...
            if (permType === 'user') {
                setSaveLoading(true);
                api.tresource.saveUserColumnPerm(values).subscribe({
                    next: () => {
                        message.success("操作成功!");
                    }
                }).add(() => setSaveLoading(false));
                ...
                setSaveLoading(true);
                api.tresource.saveUsetColumnPerm(values).subscribe({
                    next: () => {
                        message.success("操作成功!");
                    }
                }).add(() => setSaveLoading(false));
    ...
    const onSelect = (record, ak) => {
        ...
        if (ak === 'buttonPerm') {
            ...
            if (permType === 'user') {
                loadUserPermEntrusts(selectedUserGroupId, selectedUserId, record.permId, props.tenantId);
            } else if (permType === 'uset') {
                loadUsetPermEntrusts(selectedUsetId, record.permId, props.tenantId);
            }
            ...
            loadTablesByPerm(selectedUserId, selectedUserGroupId, record.permId, () => {
                if (permType === 'user') {
                    loadUserDataPerm(selectedUserId, selectedUserGroupId, record.permId, props.tenantId);
                } else if (permType === 'uset') {
                    loadUsetDataPerm(selectedUsetId, record.permId, props.tenantId);
                }
            });
            ...
            loadColumnPermTablesByPerm(record.permId, () => {
                if (permType === 'user') {
                    loadUserColumnPerm(selectedUserId, selectedUserGroupId, record.permId, props.tenantId);
                } else if (permType === 'uset') {
                    loadUsetColumnPerm(selectedUsetId, record.permId, props.tenantId);
                }
            });
    ...
    const onUserSelect = (node) => {
        ...
        loadPermResourceByUser(node.parentId, uid, props.tenantId);
    ...
    const onUsetSelect = (node) => {
        ...
        loadPermResourceByUset(usetId, props.tenantId);
        setPermType('uset');
    ...
    const submitUserPerm = () => {
        ...
        form.validateFields().then((values) => {
            ...
            if (permType === 'user') {
                api.tresource.saveUserBusinessPerm(values).subscribe({
                    next: () => {
                        message.success('操作成功!');
                        loadPermResourceByUser(selectedUserGroupId, selectedUserId, props.tenantId);
                    },
                }).add(() => setConfirmLoading(false));
            } else if (permType === 'uset') {
                api.tresource.saveUsetBusinessPerm(values).subscribe({
                    next: () => {
                        message.success('操作成功!');
                        loadPermResourceByUset(selectedUsetId, props.tenantId);
                    },
                }).add(() => setConfirmLoading(false));
            }
        }).catch(() => {
    ...
    const treeAllUset = (tid) => {
        api.tuset.treeAllUset(tid).subscribe({
            ...
            <Row gutter={5}>
                ...
                <Col span={10}>
                    <Card
                        ...
                        bordered={true}
                        bodyStyle={{ height: offsetHeight - 103, overflow: 'auto' }}
                    >
                        <Table
                            size="small"
                            bordered
                ...
                {selectedTag && selectedTag === 'BUTTON' && (
                    <Col span={9}>
                        <Card
                            ...
                            size='small'
                            bordered={true}
                            bodyStyle={{ padding: 5, height: offsetHeight - 66, overflow: 'auto' }} >
                            <Tabs
                                activeKey={key}
                                size="small"
                                type="card"
