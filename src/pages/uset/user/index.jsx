...
export default (props) => {
    ...
    const loadGroupUsersByUset = (usetId) => {
        api.user.listGroupUsersByUset(usetId).subscribe({
            next: (data) => {
                const ids = [];
                forEach((v) => {
                    let groups = [];
    ...
    useEffect(() => {
        setCurrent({usetId:params.id});
        loadGroupUsersByUset(params.id);
    ...
    const [onSaveClick] = useAutoObservableEvent([
        ...
        switchMap((userUset) => api.uset.saveUserUset(userUset)),
        tap(() => {
