...
export default (props) => {
    ...
    const [onClickMenu] = useAutoObservableEvent(
        [
            tap(() => setLoading(true)),
            switchMap(([menuId, userId, groupId]) => zip(api.role.listAllButtonsByMenu(menuId), api.user.listPermButtons(userId, groupId, menuId))),
    ...
    const onMenuSave = (privileges) => {
        api.user.saveMenuPerm(privileges).subscribe({
            next: () => {
    ...
    const onButtonSave = (privileges) => {
        api.user.saveButtonPerm(privileges).subscribe({
            next: () => {
                message.success('操作成功!');
            }
        })
                                    ...
                                    <Button
                                        ...
                                        onClick={() => {
                                            ...
                                            const privileges = {
                                                ...
                                                menuId,
                                                userId: current.userId, groupId: current.groupId,
                                            };
