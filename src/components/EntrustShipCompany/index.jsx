...
const EntrustShipCompany = (props) => {
  const { Option } = Select;

  const { value, displayName, placeholder, style, getEntrustShipCompany, onChange } = props;
  ...
  const [onSearch, optionData, setOptionData] = useObservableAutoCallback((event) =>
    event.pipe(
      ...
      switchMap((v) => api.entrustShipCompany.listByKeyword(v || '')),
    ),
  );
  ...
  const [doOnChange] = useObservableAutoCallback((event) =>
    event.pipe(
      ...
      tap((v) => getEntrustShipCompany && getEntrustShipCompany(v || {})),
    ),
  );
...
export default EntrustShipCompany;
